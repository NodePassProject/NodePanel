...
export function AppLogo({ className }: AppLogoProps) {
  ...
  useEffect(() => {
    if (resolvedTheme) {
      setCurrentLogo(resolvedTheme === 'dark' ? darkModeSvg : lightModeSvg);
    }
  }, [resolvedTheme, darkModeSvg, lightModeSvg]);

  if (!currentLogo) {
    // Fallback or initial render before theme is resolved
    return <div className={className || 'h-8 w-8'} aria-label="Loading logo..." />;
  }

  return <div dangerouslySetInnerHTML={{ __html: currentLogo }} />;
}
